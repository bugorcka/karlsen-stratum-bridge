...
import (
	...
	"github.com/karlsen-network/bugorcka/nautilusstratum/src/karlsenstratum"
	"gopkg.in/yaml.v2"
)
...
func main() {
	...
	flag.StringVar(&cfg.StratumPort, "stratum", cfg.StratumPort, "stratum port to listen on, default `:1555`")
	flag.BoolVar(&cfg.PrintStats, "stats", cfg.PrintStats, "true to show periodic stats to console, default `true`")
	flag.StringVar(&cfg.RPCServer, "karlsen", cfg.RPCServer, "address of the karlsend node, default `localhost:38110`")
